...
const ENROLL_REQUIRED_GOOD_FRAMES = 3;
const ENROLL_PHASE_META = {
    front: { label: 'FRONT', instruction: 'Look straight into the camera', arrow: 'arrow_upward', guideClass: 'front' },
    left: { label: 'LEFT', instruction: 'Turn your face to the LEFT', arrow: 'arrow_back', guideClass: 'left' },
    right: { label: 'RIGHT', instruction: 'Turn your face to the RIGHT', arrow: 'arrow_forward', guideClass: 'right' }
};
let enrollState = {
    id: null, name: null, className: null,
    stream: null, currentPhase: 0, autoTimer: null, isValidating: false,
    ...
    try {
        ...
        if (!data.history || data.history.length === 0) {
            ...
            data.history.forEach(h => {
                ...
                const status = (h.status || 'present').toLowerCase();
                const scannedAt = h.scanned_at || h.session_date || h.created_at;
                el.innerHTML = `
                    <div><p style="font-weight:600;color:var(--on-surface)">${h.session_name || 'Session ' + h.session_id}</p><p style="font-size:0.75rem;color:var(--on-surface-variant)">${formatDateTime(scannedAt)}</p></div>
                    <span class="status-chip ${status === 'present' ? 'active' : 'archived'}">${status}</span>
                `;
                hist.appendChild(el);
            });
        }
...
function proceedToCameraEnroll() {
    ...
    if (!s) return;
    enrollState = { ...s, currentPhase: 0, stream: null, autoTimer: null, isValidating: false, captures: {front:[], left:[], right:[]} };
    ...
    setEnrollAutoStatus('Auto capture is checking your face...');
...
function startAutoEnrollCapture() {
    ...
    enrollState.autoTimer = setInterval(() => captureEnrollFrame('auto'), 1100);
    setEnrollAutoStatus('Auto capture is checking your face...');
}

function stopAutoEnrollCapture() {
    if (enrollState.autoTimer) {
        clearInterval(enrollState.autoTimer);
        enrollState.autoTimer = null;
    }
}

function getEnrollCaptures(angle) {
    const captures = enrollState.captures[angle];
...
function updateEnrollUI() {
    ...
    document.getElementById('enroll-capture-btn').disabled = enrollState.isValidating;
    document.getElementById('enroll-submit-btn').disabled = true;
    document.getElementById('enroll-instruction').textContent =
        `${meta.instruction}. Auto capture: ${count}/${ENROLL_REQUIRED_GOOD_FRAMES}`;
...
function setEnrollThumbnail(phase, blob, state = 'validating', reason = '') {
    ...
    card.querySelector('.thumb-check').style.display = state === 'success' ? 'flex' : 'none';
    card.querySelector('.thumb-overlay p').textContent = state === 'error' ? `FAILED: ${reason}` : 'RETAKE';
}

async function captureEnrollFrame(source = 'manual') {
    const phase = ENROLL_PHASES[enrollState.currentPhase];
    if(!enrollState.stream || !phase || enrollState.isValidating) return;

    const blob = await captureEnrollBlob();
    if (!blob) return;

    enrollState.isValidating = true;
    document.getElementById('enroll-capture-btn').disabled = true;
    setEnrollAutoStatus(`Checking ${ENROLL_PHASE_META[phase].label} angle...`);
    if (source === 'manual') {
        flashCamera();
    ...
    try {
        ...
        if(data.success) {
            ...
            if (captures.length < ENROLL_REQUIRED_GOOD_FRAMES) {
                captures.push(blob);
                enrollState.captures[phase] = captures;
            }
            ...
            // Advance only after enough good frames for this angle.
            if (count >= ENROLL_REQUIRED_GOOD_FRAMES) {
                enrollState.currentPhase++;
                while(
                    enrollState.currentPhase < ENROLL_PHASES.length &&
                    getEnrollCaptureCount(ENROLL_PHASES[enrollState.currentPhase]) >= ENROLL_REQUIRED_GOOD_FRAMES
                ) {
                    enrollState.currentPhase++;
                }
                if (enrollState.currentPhase < ENROLL_PHASES.length) {
                    setEnrollAutoStatus(`${label} complete. Moving to next angle...`, 'success');
                }
            }
        } else {
            const reason = data.reason || 'Angle not ready';
            if (source === 'manual') {
        ...
        if (source === 'manual') showToast("Validation request failed", "error");
        setEnrollAutoStatus('Validation request failed. Retrying...', 'error');
    } finally {
        enrollState.isValidating = false;
        updateEnrollUI();
    }
}
